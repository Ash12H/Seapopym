...
from seapopym.configuration.no_transport import FunctionalGroupUnit, FunctionalTypeParameter, MigratoryTypeParameter
...
def _as_dataset__load_forcings(forcing_parameter: ForcingParameter) -> xr.Dataset:
    """Return the forcings as a xarray.Dataset."""
    all_forcing = {
        forcing_key: forcing_value["forcing"]
        for forcing_key, forcing_value in attrs.asdict(forcing_parameter).items()
        if forcing_value is not None and forcing_key not in ["timestep", "resolution"]
    }
    all_forcing[ConfigurationLabels.timestep] = forcing_parameter.timestep
    all_forcing[ConfigurationLabels.resolution_latitude] = forcing_parameter.resolution[0]
    all_forcing[ConfigurationLabels.resolution_longitude] = forcing_parameter.resolution[1]
...
def _as_dataset__build_fgroup_dataset__generate_param_coords(
    ...
    grps_param = _rec_parameters([attrs.asdict(grp) for grp in functional_group])
...
def _as_dataset__build_fgroup_dataset(functional_group: list[FunctionalGroupUnit]) -> xr.Dataset:
    """
    ...
    """
    (param_as_dict, f_group_coord) = _as_dataset__build_fgroup_dataset__generate_param_coords(functional_group)
    names_classes = [
...
def _as_dataset__build_cohort_dataset(functional_group: list[FunctionalGroupUnit], names: xr.DataArray) -> xr.Dataset:
    """Return the cohort parameters as a xarray.Dataset."""
    all_cohorts_timesteps = [fgroup.functional_type.cohorts_timesteps for fgroup in functional_group]
    all_index = [names[CoordinatesLabels.functional_group][names == fgroup.name] for fgroup in functional_group]
    return xr.merge(
        [
...
def as_dataset(functional_group: list[FunctionalGroupUnit], forcing_parameter: ForcingParameter) -> xr.Dataset:
    """Return the configuration as a xarray.Dataset."""
    fgroup = _as_dataset__build_fgroup_dataset(functional_group=functional_group)
    forcings = _as_dataset__load_forcings(forcing_parameter)
    cohorts = _as_dataset__build_cohort_dataset(functional_group, fgroup[ConfigurationLabels.fgroup_name])
