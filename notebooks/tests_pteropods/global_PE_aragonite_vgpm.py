    ...
    for idx, row in df.iterrows():
        f_group = FunctionalGroupUnit(
                    ...
                    functional_type=FunctionalTypeParameter(
                        ...
                        lambda_acidity_0=row['lambda_ar_0'],
                        gamma_lambda_acidity=row['gamma_lambda_ar'],
                        tr_0=row['tr_0'],
...
def common_mask(primary_production,temperature,acidity,ds_pld):
    ...
    with xr.set_options(keep_attrs=True):
        ...
        # aragonite
        acidity = acidity.where(mask)
        # primary production 
        primary_production= primary_production.where(mask)
        # pealgic layer depth
        ds_pld = ds_pld.where(mask)
    return primary_production,temperature,acidity,ds_pld
...
def config_model(f_group_path,NPP_product,ds):
    ...
    p_param = ForcingParameter(
        ...
        acidity=ForcingUnit(forcing=ds["acidity"])
    )
...
def main():
    ...
    acidity=format_variable(da_ar,dimension=4)
    ...
    if name_npp=='vgpm':
        ...
        primary_production,temperature,acidity,ds_pld=common_mask(primary_production,temperature,acidity,ds_pld)
    ...
    # Libérer la mémoire
    del biomass, biomass_m3, model, dataset, primary_production, acidity, temperature
    gc.collect()
